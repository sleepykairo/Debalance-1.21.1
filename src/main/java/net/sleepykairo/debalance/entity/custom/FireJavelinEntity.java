...
public class FireJavelinEntity extends PersistentProjectileEntity {
    private static final TrackedData<Byte> LOYALTY = DataTracker.registerData(FireJavelinEntity.class, TrackedDataHandlerRegistry.BYTE);
    private static final TrackedData<Boolean> ENCHANTED = DataTracker.registerData(FireJavelinEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
    private boolean dealtDamage;
    public int returnTimer;

    public FireJavelinEntity(EntityType<? extends FireJavelinEntity> entityType, World world) {
        super(entityType, world);
    }

    public FireJavelinEntity(World world, LivingEntity owner, ItemStack stack) {
        super(ModEntities.FIRE_JAVELIN_ENTITY, owner, world, stack, null);
        this.dataTracker.set(LOYALTY, this.getLoyalty(stack));
        this.dataTracker.set(ENCHANTED, stack.hasGlint());
    }

    public FireJavelinEntity(World world, double x, double y, double z, ItemStack stack) {
        super(ModEntities.FIRE_JAVELIN_ENTITY, x, y, z, world, stack, stack);
        this.dataTracker.set(LOYALTY, this.getLoyalty(stack));
        this.dataTracker.set(ENCHANTED, stack.hasGlint());
    }
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        getWorld().addParticle(ModParticles.FIRE_JAVELIN_EXPLOSION_PARTICLE,
                entityHitResult.getPos().getX(), this.getPos().getY(), this.getPos().getZ(),
                0, 0, 0);
    }
