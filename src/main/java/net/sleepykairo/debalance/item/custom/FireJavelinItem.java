...
public class FireJavelinItem extends Item implements ProjectileItem {
    ...
    public FireJavelinItem(Item.Settings settings) {
        super(settings);
    }
                ...
                if (!(f > 0.0F) || playerEntity.isTouchingWaterOrRain()) {
                    if (!isAboutToBreak(stack)) {
                        ...
                        if (!world.isClient) {
                            ...
                            if (f == 0.0F) {
                                FireJavelinEntity fireJavelinEntity = new FireJavelinEntity(world, playerEntity, stack);
                                fireJavelinEntity.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0F, 2.5F, 1.0F);
                                if (playerEntity.isInCreativeMode()) {
                                    fireJavelinEntity.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                                }

                                world.spawnEntity(fireJavelinEntity);
                                world.playSoundFromEntity(null, fireJavelinEntity, registryEntry.value(), SoundCategory.PLAYERS, 1.0F, 1.0F);
                                if (!playerEntity.isInCreativeMode()) {
